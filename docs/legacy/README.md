...
| `statusline-command.sh` | a primeira de todas (junho), estilo Starship; nunca versionada |

Eram quatro arquivos no `~/.claude`, não três: a `statusline-1.sh` era cópia byte
a byte da `statusline.sh` e não foi arquivada.
